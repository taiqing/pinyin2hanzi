...
def main():
    ...
    for l in range(n_layer):
        ...
        variables = join_dicts([variables, encoder.variables])
        encoder_layers.append(encoder)
    W_o = weight_variable_uniform([n_hidden[-1], n_output], weight_stddev)
    ...
    outputs = list()
    for t in range(n_step_input):
        h_t = tf.concat(1, states_layers[-1][t])
